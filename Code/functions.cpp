...
extern "C" {
    ...
    class Subregion {
        ...
        boost::unordered_map<std::vector<int>, boost::unordered_map<std::vector<int>, std::vector<int>>> instance_table;
        boost::unordered_map<std::vector<int>, boost::unordered_map<int, boost::container::flat_set<int>>> hashmap;
    ...
    std::map<int, std::vector<int>> calc_starNeighbors(std::vector<Feature> features) {
        ...
        boost::container::flat_set<int> feature_types;
        ...
        for(int current_feature : feature_types) {
            ...
            for(size_t i = 0; i < current_feature_coords.size(); ++i) {
                ...
                for (const auto& neighbor : nearby_points) {
                    ...
                    if (neighbor_id > current_feature_coords[i].id && calc_distance(current_feature_coords[i], 
                                                                                    neighbor_feature) <= distance_threshold) {
    ...
    std::vector<std::vector<int>> colocationGeneral(        
        std::vector<std::vector<int>> candidatePatterns, double prevalence_threshold, int degree, int number_subregions, 
        boost::unordered_map<std::vector<int>, boost::unordered_map<std::vector<int>, std::vector<int>>>& instance_table, 
        boost::unordered_map<std::vector<int>, boost::unordered_map<int, boost::container::flat_set<int>>>& hashmap, 
        std::map<int, std::vector<int>> starNeighbors, std::map<int, FeatureInformation> featureInfo) {

        std::vector<std::vector<int>> prevalent;
        prevalent.reserve(candidatePatterns.size());  // Reserve space in advance

        for (const auto& currentPattern : candidatePatterns) {
            std::vector<int> basePattern(currentPattern.begin(), currentPattern.begin() + degree - 1);
            ...
            boost::unordered_map<std::vector<int>, std::vector<int>>& colocTable = colocTableIt->second;

            for (const auto& entry : colocTable) {
                const std::vector<int>& key = entry.first;
                boost::container::flat_set<int> commonLastNeighbors;

                // Cache starNeighbors lookup outside loop
                for (int instanceID : key) {
                    ...
                    if (commonLastNeighbors.empty()) {
                        ...
                        std::vector<int> temp_vector = findNeighborsInRange(star_neighbor_it->second, 
                                                                            featureInfo[lastFeature].start,
                                                                            featureInfo[lastFeature].end);
                        boost::container::flat_set<int> temp_commonLastNeighbors(temp_vector.begin(), temp_vector.end());
                        boost::range::set_intersection(commonLastNeighbors, temp_commonLastNeighbors, std::inserter(commonLastNeighbors, commonLastNeighbors.begin()));
                    }
                }

                // Reuse commonLastNeighbors for each colocTable key
                for (int n : colocTable[key]) {
                    ...
                    std::vector<int> temp_vect = findNeighborsInRange(star_neighbor_it->second,
                                                                      featureInfo[lastFeature].start,
                                                                      featureInfo[lastFeature].end);
                    boost::container::flat_set<int> temp_neighbors(temp_vect.begin(), temp_vect.end());
                    boost::container::flat_set<int> neighbors;
                    boost::range::set_intersection(commonLastNeighbors, temp_neighbors, std::inserter(neighbors, neighbors.begin()));
            ...
            for (int m = 0; m < degree; ++m) {
                ...
                double ratio = static_cast<double>(hashmap[currentPattern][f].size()) / featureInfo[f].count;
                pr.push_back(ratio);
            }

            // Find minimum prevalence
        ...
        //std::cout << "keys: " << num_keys << " values: " << values << std::endl;
    ...
    bool allSubpatternsInPrevalent(const std::vector<int>& pattern, 
                                   const boost::unordered_set<std::vector<int>>& prevalentPatterns, 
                                   int degree) {
    ...
    std::vector<std::vector<int>> getCandidatePatterns(const std::vector<std::vector<int>>& prevalentPattern, int degree, const std::map<int, FeatureInformation> featureInfo) {
        boost::unordered_set<std::vector<int>> prevalentPatterns(prevalentPattern.begin(), prevalentPattern.end());
    ...
    std::vector<std::vector<int>> degree2Processing(std::vector<std::vector<int>> candidatePatterns, 
                                                    ...
                                                    std::map<int, FeatureInformation> featureInfo,
                                                    std::map<int, std::vector<int>> starNeighbors,
                                                    boost::unordered_map<std::vector<int>, boost::unordered_map<std::vector<int>,
                                                        std::vector<int>>> &instance_table, 
                                                    boost::unordered_map<std::vector<int>,
                                                        boost::unordered_map<int, boost::container::flat_set<int>>> &hashmap) {   
        std::vector<std::vector<int>> size2_patterns;
    ...
    void subregion_main() {
        std::string input_file_path = "/Data/Region6";
        ...
        for(int i = 0; i < number_subregions; i++) {
            ...
            for (auto it = subregions[i].instance_table.begin(); it != subregions[i].instance_table.end(); ) {
                if (it->first.size() > 2) {
                    it = subregions[i].instance_table.erase(it); // Erase and move to the next iterator
                } else {
    ...
    void border_main(int number_borders) {
        ...
        std::string input_file = "/IntermediateData/border.csv";
        ...
        for (int i = 0; i < number_borders; i++) {
            ...
            borders[i].size2_patterns = degree2Processing(size2_candidatePatterns,
                                                          prevalence_threshold, i, 
                                                          borders[i].featureInfo,
                                                          borders[i].starNeighbors,
                                                          borders[i].instance_table,
                                                          borders[i].hashmap);
        } 
        return;
    }
    ...
    void update_border_info(int* ids, int ids_size, int i) {
        ...
        boost::unordered_map<std::vector<int>, boost::unordered_map<int, boost::container::flat_set<int>>> temp_hash;
        for (const auto& outer_pair : borders[i].hashmap) {
            ...
            boost::unordered_map<int, boost::container::flat_set<int>> temp_inner_map;
            for (const auto& inner_pair : inner_map) {
                const int& inner_key = inner_pair.first;
                const boost::container::flat_set<int>& inner_set = inner_pair.second;
                boost::container::flat_set<int> temp_inner_set;
        ...
        boost::unordered_map<std::vector<int>, boost::unordered_map<std::vector<int>, std::vector<int>>> temp_instance_table;

        for (const auto& outer_pair : borders[i].instance_table) {
            const std::vector<int>& outer_key = outer_pair.first;
            const auto& inner_map = outer_pair.second;
            boost::unordered_map<std::vector<int>, std::vector<int>> temp_inner_map;
            for (const auto& inner_pair : inner_map) {
    ...
    std::vector<std::vector<int>> region_generate_size2_combos() {
        boost::unordered_set<std::vector<int>> patterns;
            ...
            for (const auto& entry : colocTable) {
                ...
                for (int instanceID : key) {
                    ...
                    if (commonLastNeighbors.empty()) {
                        ...
                        set_intersection(commonLastNeighbors.begin(), commonLastNeighbors.end(),
    ...
    void region_main() {
        boost::container::flat_set<int> set_features;
